...
static var _for_nodes: Dictionary[StringName, Variant]
...
static func finish(properties := {}, print_source := false) -> GDScript:
	var head := ""
	var gd := GDScript.new()
	...
	for prop in _for_nodes:
		gd.for_nodes[prop] = _for_nodes[prop]
	_source_code = ""
	_method_count = 0
	return gd

static func add_subnode(node: Node) -> StringName:
	var uid := "node_%s" % len(_for_nodes)
	_for_nodes[uid] = node
	return uid
